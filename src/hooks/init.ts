...
import { DocumentText, FileTray, Image, LogoChrome, CodeOutline, PlayCircle } from "@vicons/ionicons5";
import { PluginGroup } from "@/class/PluginCenter";
...
export default function init() {
	...
	pluginCenter.registerComponent({
		...
		name: "视频",
		icon: PlayCircle,
		getComponent: async () => (await import("@/views/miaoVideoPlayer.vue")).default,
		filter: (vDirs, vFiles) => {
			if (vDirs.length !== 0) {
				return false;
			}
	...
	pluginCenter.registerComponent({
		...
		name: "框架",
		icon: LogoChrome,
		getComponent: async () => (await import("@/views/miaoIframe.vue")).default,
		filter: (vDirs, vFiles) => {
			if (vDirs.length !== 0) {
				return false;
			}
	...
	pluginCenter.registerComponent({
		...
		name: "代码编辑",
		icon: CodeOutline,
		getComponent: async () => (await import("@/views/miaoCodeMirror.vue")).default,
		filter: (vDirs, vFiles) => {
			if (vDirs.length !== 0) {
				return false;
			}
			...
			for (let end of ["js", "ts", "jsx", "tsx", "py", "html", "css", "scss", "go", "json", "md", "yaml", "txt", "xml", "vue"]) {
	...
	pluginCenter.registerComponent({
		...
		name: "图片浏览",
		icon: Image,
		getComponent: async () => (await import("@/views/miaoImages.vue")).default,
		filter: (vDirs, vFiles) => {
	...
	pluginCenter.register("test", {
		...
		func: async (__, vfs) => {
			const c = (await import(/* @vite-ignore */ vfs[0].url)).default;
			console.log(c);
			pluginCenter.registerComponent({
				key: `test-${Math.random()}`,
				name: `test-${Math.random()}`,
				icon: DocumentText,
				getComponent: async () => (await import(/* @vite-ignore */ vfs[0].url)).default,
				filter: (vDirs, vFiles) => {
					if (vDirs.length !== 0) {
						return false;
					}
